...
func _ready():
	...
	\u25cf每回合可激活1次，移动范围1，能够破坏遇到的角色
	\u25cf每回合结束时，消耗1点鱼，如果鱼不足则被破坏
	【人类】"""
...
func activate():
	...
	GameManager.set_prompt("选择一个地面位置进行移动，破坏遭遇的角色")
	...
	if !GameManager.grid_at(result[0],result[1]).flag_sea and d > 0 and d <= 1 :
		#选择成功
